...
// 0 - details
// 1 - candidate
const EditMeeting = ({ currentGuest, socket }) => {
  ...
  const processParams = async () => {
    ...
    const editFromParams = parseInt(
      new URLSearchParams(window.location.search).get("edit")
    );
    const meetingIDFromParams = new URLSearchParams(window.location.search).get(
      "meetingID"
    );
    ...
    if (!meetingIDFromParams) {
      ...
      meetingIDFromParams.length !== 6 ||
      !(editFromParams === 0 || editFromParams === 1)
    ) {
      redirect("/join", [{ key: "meetingID", value: meetingIDFromParams }]);
      return;
    }

    await meetingExists(meetingIDFromParams).then((response) => {
