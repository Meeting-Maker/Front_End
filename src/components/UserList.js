  ...
  function UserCard(user) {
    ...
    if (selectedCandidate) {
      selectedStyle =
        ...
        selectedCandidate.voters.filter((voter) => voter.userID === user.id)
          .length > 0
          ? "teal"
          : "";
    } else {
    ...
    return (
      <div
        key={user.id}
        onClick={() =>
          onSelectUser({ id: user.id, name: user.name, role: user.role })
        }
        style={{ padding: "0.5rem 0 0.5rem 0" }}
      >
        <div className="ui container" style={{ width: "32rem" }}>
          <div className={`ui ${selectedStyle} fluid card user-card`}>
            <div className="content">
              <div className={"ui header floated left"}>{user.name}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div>
      <div
        ...
        style={{
          ...
          marginTop: "1rem",
          maxHeight: votingPage ? "60%" : "80%",
        }}
      >
        <div
          className={"ui medium header"}
          style={{ margin: "0.5em 0 0 0", textAlign: "center" }}
        >
          Owners
        </div>
        {renderedAdminList}
