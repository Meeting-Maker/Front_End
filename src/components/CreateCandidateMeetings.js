...
const CreateCandidateMeetings = ({
   ...
   const [dueDateTTime, setDueDateTTime] = useState("");
   ...
   getMeetingDetails(meetingID).then((response) => {
      setDueDateTTime(response.data.meetingDetails.dueDate);
   });
   ...
   const validateCandidateMeeting = () => {
      ...
      if(!isAfterDate({
         input: date + "T" + time + ":00",
         compareAgainst: dueDateTTime}
      )){
         tempErrors.push("Meeting Option must be after when the response is due.")
      }
         ...
         <div className="ui grey centered card" style={{width: "30rem"}}>
            <div className="content">
              ...
              <Tooltip top={"-0.25%"} left={"102%"} width={"18rem"}>
                Enter multiple different options (at least 2) for when to hold
                your meeting. People with the meeting link will be able to vote
                on which meeting time they prefer.
              </Tooltip>
            </span>
               </div>
            </div>
            <div className="content">
               <form
                  id="createCandidateMeetingsForm"
